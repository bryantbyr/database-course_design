    ...
    ui(new Ui::Handler)
{
    ...
    connect(ui->group,SIGNAL(triggered()),this,SLOT(setGroupSize()));
    ...
    connect(ui->exportResult,SIGNAL(triggered()),this,SLOT(exportResultData()));
    ...
    QString dsn = QString::fromLocal8Bit("TD-LTE");      //数据源名称
...
void Handler::setGroupSize(){
    QInputDialog d;
//    d.setFixedHeight(100);
//    d.setFixedWidth(200);
//    d.setGeometry(0,0,2000,100);
    groupSize = d.getInt(this,"数据导入","分组大小",0,50,1000,1);
}
...
{    
    ...
    while(!in.atEnd())
    {
        for(int i=0;!in.atEnd()&&i<groupSize;i++){
            QString fileLine = in.readLine();
            list = fileLine.split(",", QString::SkipEmptyParts);
            if(list.size()>6){
                bool isValid = true;
                for(int k=0;k<7;k++){//检查属性值是否为空
                    if(list.at(k).isEmpty())
                        isValid = false;
                }
                if(isValid){
        ...
        TimeStamp.clear();
        ServingSector.clear();
        InterferingSector.clear();
        LteScRSRP.clear();
        LteNcRSRP.clear();
        LteNcEarfcn.clear();
        LteNcPci.clear();

        ui->bar->setValue(int(j+=groupSize));
        QCoreApplication::processEvents();
...
void Handler::inserttbCellExcelData()
{
    ...
    for(int i=1; i< intRows; i++)
    {
        ...
        if(allEnvDataList_i.size()>18){
            ...
            bool isValid = true;//检查合法性
            if(allEnvDataList_i[6].toDouble()>503||allEnvDataList_i[6].toDouble()<0){
                isValid = false;
            }
            if(allEnvDataList_i[7].toDouble()>2||allEnvDataList_i[7].toDouble()<0){
                isValid = false;
            }
            if(allEnvDataList_i[8].toDouble()>167||allEnvDataList_i[8].toDouble()<0){
                isValid = false;
            }
            if(ok==true&&isValid==true){
...
void Handler::insertPRBExcelData()
{
    ...
    for(int i=2; i< intRows+1; i++)
    {
        ...
        bool isValid = true;
        if(allEnvDataList_i[1].toDouble()!=15){
            isValid = false;
        }
        if(isValid){
...
void Handler::exportResultData(){
    if(ui->monitoirView->verticalHeader()->count()==0){
        QMessageBox::warning(0,"警告","导出内容不能为空！",QMessageBox::Ok);
        return;
    }
    QString result;
    QAbstractItemModel *modelTest = ui->monitoirView->model();
    int rows = modelTest->rowCount();
    int columns = modelTest->columnCount();

    if(columns==1){
        result=QString("基站信息查询");
    }
    else if(columns==19){
        result=QString("小区配置信息查询");
    }
    ...
    QList<QVariant> t;
    for(int n = 0;n < columns;n++)
        t.append(modelTest->headerData(n,Qt::Horizontal));
    vars.append(QVariant(t));
    for (int i = 0; i < rows; ++i) {
        t.clear();
        for(int n = 0;n < columns;n++)
            t.append(modelTest->data(modelTest->index(i,n)));
        vars.append(QVariant(t));
        if(i==0){
            QVariant var = QVariant(vars);
            QString rangStr;
